...
def detect_needle_angle(
    ...
    if is_temperature:
        # 温度計（画像全体の上半分）
        start_y = max(0, cy - 120)
        end_y = cy + 20
        search_radius = 80
    else:
        # 湿度計（画像全体の下半分）
        start_y = cy - 20
        end_y = min(image.shape[0], cy + 120)
        search_radius = 60
    ...
    for angle_deg in range(0, 360, 10):
        ...
        for r in range(10, search_radius):
            x = int(cx + r * math.cos(angle_rad))
            y = int(cy + r * math.sin(angle_rad))

            # 画像範囲内かチェック
            if 0 <= x < image.shape[1] and start_y <= y < end_y:
                # 赤い色の強度を調べる
                bgr = image[y, x]
                b, g, r = bgr

                # 赤い色の強度を計算（赤が強く、青・緑が弱い）
                red_strength = int(r) - max(int(g), int(b))
                if red_strength > 30:  # 赤い色のしきい値
                    score += red_strength
                    needle_points.append((x, y))

        # 針の連続性もチェック
        if len(needle_points) > 5:  # 最低5点以上の連続点
            score += len(needle_points) * 10
